#Author - Not Me. Holding here for now. Will get with Author to see if they want name published.
CLS
...
$URI = "https://graph.microsoft.com/beta/users?`$select=displayName,userPrincipalName, mail, id, assignedLicenses, licenseDetails, CreatedDateTime, signInActivity, UserType&`$top=999"
...
Write-Host "All done. " $Report.Count "accounts processed - output available in c:\Temp\FILE.csv."
$Report | Sort DisplayName | Out-GridView
$Report | Export-CSV -NoTypeInformation c:\Temp\FILE.csv